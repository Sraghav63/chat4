...
export async function syncClerkUser(
  ...
  try {
    ...
    throw new ChatSDKError('bad_request:database', 'Failed to sync Clerk user');
  }
}

export async function getUserByClerkId(clerkId: string): Promise<User | null> {
  try {
    const result = await client.query(api.queries.getUserByClerkId, {
      clerkId,
    });
